	...
	namespace Wheel
	{
		void Update()
		{
			...
			*/
			...
			tileMain->Set("_Text", name);
			tileMain->Set("_Slice", selectedHotkey + 1);
			...
			for (UInt8 i = 0; i < InventoryChanges::kHotkeyStewie; i++)
			{
				...
				if (const auto [item, itemExtra] = InventoryChanges::HotkeyGet(i); item)
				{
					...
					if (icon) image->Set("_ImageFilename", icon->ddsPath.CStr());
					image->Set("_Image", true);
					...
					if (extraAmmo && extraAmmo->ammo)
					{
						...
						image->Set("_AmmoCount", ammoCount);
						image->Set("_AmmoName", extraAmmo->ammo->abbreviation.CStr());
					}
					else
					{
						image->Set("_AmmoCount", "");
						image->Set("_AmmoName", "");
					}
				}
				else
				{
					image->Set("_Image", false);
				}
			}
		}
		...
		void Activate()
		{
			...
			g_HUDMainMenu->tileInfo->Set(kTileValue_visible, false);
			Update();
			tileMain->Set("_Visible", true);
		}
		...
		void Deactivate()
		{
			...
			g_HUDMainMenu->tileInfo->Set(kTileValue_visible, true);

			tileMain->Set("_Visible", false);
		}
...
//		tileMain->SetFloat("_Color", systemcolor);
		...
		tileMain->Set("_AlphaMult", alphaMult);
		tileMain->Set("_WidthBase", width);
		tileMain->Set("_HeightBase", height);
		tileMain->Set("_OffsetHorizontal", offsetHeight);
		tileMain->Set("_OffsetVertical", offsetWidth);
