...
public:
	...
	struct TargetData
	{
		...
		UInt8				pad[3];			// 01
		ObjectType			kTarget;		// 04
		UInt32				uiCount;		// 08 can be distance too
		float				unk0C;			// 0C
	...
	UInt32			uiProcedureArrayIndex;	// 018 index into array of array of eProcedure terminated by 0x2C.
	//	   -1 if no procedure array exists for package type.
	EnumPackageFlag	ePackageFlags;		// 01C
	EnumPackageType	eType;				// 020
	UInt8			pad021[1];			// 021
	UInt16			usBehaviorFlags;	// O22
	UInt32			uiSpecificFlags;	// 024
	TESPackageData* pkPackageData;		// 028
	LocationData*	pkLocation;			// 02C
	TargetData*		pkTarget;			// 030	target ?
	UInt32			unk034;				// 034	idles
	PackageTime		kTime;				// 038
	UInt32			unk040;
	UInt32			unk044;
	UInt32			unk048;
